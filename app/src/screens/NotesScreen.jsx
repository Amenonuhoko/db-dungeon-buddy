...
import { PersonalBoard } from '../components/PersonalBoard.jsx';
...
export function NotesScreen() {
  ...
  // Typed notes, or your private sketch board (013) — kept in the URL so
  // coming back to Notes lands where you were.
  const [params, setParams] = useSearchParams();
  const view = params.get('view') === 'board' ? 'board' : 'notes';
  const showView = (next) => setParams(next === 'board' ? { view: 'board' } : {}, { replace: true });
  ...
  return (
    <div>
      <div className="party-views" role="tablist" aria-label="Notes">
        <button type="button" role="tab" aria-selected={view === 'notes'} className={view === 'notes' ? 'active' : ''} onClick={() => showView('notes')}>
          Notes
        </button>
        <button type="button" role="tab" aria-selected={view === 'board'} className={view === 'board' ? 'active' : ''} onClick={() => showView('board')}>
          My Board
        </button>
      </div>

      {view === 'board' ? (
        <PersonalBoard status={status} campaignId={campaignId} userId={isGuest ? 'local' : user?.id} />
      ) : (
        <>
